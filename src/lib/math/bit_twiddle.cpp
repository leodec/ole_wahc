...
namespace lbcrypto {
    ...
    inline static constexpr unsigned char reverse_byte(unsigned char x)
    {
        // static
    ...
    // void ReverseBits(ui32 * input, ui32 size, ui32 msb) {

    // }

    ui32 log_pow2(ui32 v){
        // Find log2 when v is a known power of two
        const ui32 b[] = {0xAAAAAAAA, 0xCCCCCCCC, 0xF0F0F0F0, 0xFF00FF00, 0xFFFF0000};
        ui32 r = (v & b[0]) != 0;
        r |= ((v & b[1]) != 0) << 1;
        r |= ((v & b[2]) != 0) << 2;
        r |= ((v & b[3]) != 0) << 3;
